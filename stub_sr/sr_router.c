                     ...
                     char* interface/* lent */)
{
    ...
    if (incoming->ip_header){
        ...
        /* Check the checksum */
        if( incoming->ip_header->ip_sum != 0xffff ) { //function probably won't be called compute_checksum, may have different arguments--it would be great to just pass it incoming and have it extract header and length
